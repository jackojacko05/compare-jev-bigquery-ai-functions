...
WITH inferred AS (
  SELECT
    *,
    `YOUR_PROJECT_ID.jev_benchmark.classify_stackoverflow_with_jev`(
      '',
      text_raw,
      'so20_jev_gateway_200_v1',
      CAST(question_id AS STRING)
    ) AS prediction
  FROM `YOUR_PROJECT_ID.jev_benchmark.so20_pilot200`
)
SELECT
  'so20_jev_gateway_200_v1' AS run_id,
  'typesafe-ai/jev' AS model,
  'gateway_equal_information' AS variant,
  question_id,
